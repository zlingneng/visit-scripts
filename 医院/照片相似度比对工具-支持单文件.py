...
class PhotoSimilarityDetector:
    ...
    def get_image_files_from_path(self, path: str) -> List[str]:
        """从路径获取图片文件，支持文件夹和单个文件"""
        image_files = []
        ...
        if os.path.isfile(path):
            ...
            if Path(path).suffix.lower() in self.supported_formats:
                ...
                print(f"添加单个文件: {os.path.basename(path)}")
            else:
    ...
    def find_similar_photos(self, source_path: str, target_paths: List[str]) -> List[Dict]:
        """分阶段查找相似照片 - 先哈希初筛，再详细比对"""
        print(f"开始分析源路径: {source_path}")
        source_files = self.get_image_files_from_path(source_path)
        print(f"源路径找到 {len(source_files)} 张照片")
        ...
        for target_path in target_paths:
            print(f"分析目标路径: {target_path}")
            files = self.get_image_files_from_path(target_path)
            target_files.extend(files)
            print(f"目标路径找到 {len(files)} 张照片")
...
def main():
    # 配置路径 - 支持文件夹和单个文件的混合配置
    # 源路径：可以是文件夹或单个图片文件
    source_path = "/Users/a000/Documents/济生/医院拜访25/2505/贵州医生拜访2505-hy/照片_transformed_20250817_225112/2_截屏2025-08-18 15.03.51.png"
    
    # 目标路径列表：每个路径可以是文件夹或单个图片文件
    target_paths = [
        # 示例：文件夹路径
        # "/Users/a000/Pictures/2502zdf2",
        ...
        # 示例：单个文件路径
        # "/Users/a000/Pictures/specific_photo.jpg",
    ...
    print("照片相似度比对工具 (支持文件夹和文件混合配置)")
    print("=" * 50)
    print(f"源路径: {source_path}")
    print(f"目标路径:")
    for target_path in target_paths:
        path_type = "文件夹" if os.path.isdir(target_path) else "文件" if os.path.isfile(target_path) else "未知"
        print(f"  - {target_path} ({path_type})")
    ...
    # 查找相似照片
    similar_pairs = detector.find_similar_photos(source_path, target_paths)
