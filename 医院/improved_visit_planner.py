...
def is_workday(date):
    ...
    # 法定节假日不可拜访
    holidays_2025 = [
        datetime(2025, 1, 1).date(), datetime(2025, 1, 28).date(), datetime(2025, 1, 29).date(),
        datetime(2025, 1, 30).date(), datetime(2025, 1, 31).date(), datetime(2025, 2, 1).date(),
        datetime(2025, 2, 2).date(), datetime(2025, 2, 3).date(),datetime(2025, 2, 4).date(), datetime(2025, 4, 4).date(),
        datetime(2025, 4, 5).date(), datetime(2025, 4, 6).date(), datetime(2025, 5, 1).date(),
        datetime(2025, 5, 2).date(), datetime(2025, 5, 3).date(), datetime(2025, 5, 4).date(),
        datetime(2025, 5, 5).date(), datetime(2025, 5, 31).date(), datetime(2025, 6, 1).date(),
        datetime(2025, 6, 2).date(), datetime(2025, 10, 1).date(), datetime(2025, 10, 2).date(),
        datetime(2025, 10, 3).date(), datetime(2025, 10, 4).date(), datetime(2025, 10, 5).date(),
        datetime(2025, 10, 6).date(), datetime(2025, 10, 7).date(), datetime(2025, 10, 8).date(),
    ]
    # 确保比较的是日期部分
...
def assign_hospitals_to_visitors(hospitals, visitors, df):
    ...
    max_diff = max(doctor_counts) - min(doctor_counts)
...
def greedy_visit_planning(df, df_addr, working_days, visitors, target_visits=400):
    """使用贪心算法制定拜访计划"""
    # 获取所有医院
    ...
    for day in working_days:
        ...
        for visitor in visitors:
            ...
            # 每人每天拜访14-17条，但不超过剩余目标
            remaining_target = visitor_targets[visitor] - visitor_visit_count[visitor]
            daily_visits = min(random.randint(14, 17), remaining_target)
            ...
            for hospital, available_count in sorted_hospitals:
                ...
                for dept, dept_doctors in dept_groups:
                    ...
                    selected_doctors = []
                    available_doctors = list(dept_doctors.iterrows())
                    random.shuffle(available_doctors)  # 随机打乱顺序
                    
                    for _, doctor_row in available_doctors:
...
def save_to_excel(visit_plan, output_file):
    """保存拜访计划到Excel文件"""
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        stats.append(['张丹凤拜访次数', len(df_result[df_result['拜访人'] == '张丹凤'])])
        stats.append(['何勇拜访次数', len(df_result[df_result['拜访人'] == '何勇'])])
...
def main():
    # 设置参数
    excel_file = '/Users/a000/Documents/济生/医院拜访25/医院医生列表2025-07-01.xlsx'
    output_file = '/Users/a000/Documents/济生/医院拜访25/2502/贵州医生拜访2502.xlsx'
    ...
    # 生成拜访计划
    print("\n正在生成拜访计划...")
    visit_plan = greedy_visit_planning(df, df_addr, working_days, visitors, target_visits)
    
    # 保存结果
    print("\n正在保存结果...")
    df_result = save_to_excel(visit_plan, output_file)
    
    # 打印统计信息
    print("\n=== 拜访计划统计 ===")
    print(f"总拜访次数：{len(visit_plan)}")
    print(f"张丹凤拜访次数：{len([v for v in visit_plan if v['拜访人'] == '张丹凤'])}")
    print(f"何勇拜访次数：{len([v for v in visit_plan if v['拜访人'] == '何勇'])}")
    ...
    for date in sorted(daily_stats.keys()):
        ...
        print(f"{date} ({weekday_name}): 张丹凤 {daily_stats[date]['张丹凤']}次, 何勇 {daily_stats[date]['何勇']}次")
